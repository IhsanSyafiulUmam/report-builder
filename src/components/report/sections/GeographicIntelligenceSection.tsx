...
interface GeographicIntelligenceSectionProps {
  section: {
    ...
    content: {
      ...
      actionTitle?: string;
      subheadline?: string;
      chartData: { Location: string; [key: string]: string }[];
...
const GeographicIntelligenceSection: React.FC<
...
> = ({ section, onUpdate, isEditable = false }) => {
  ...
  const locationData = chartData.map(
    (row: { Location: string; [key: string]: string }) => {
  ...
  const renderLocationChart = (data: Record<string, unknown>[]) => {
    return (
  ...
  return (
    ...
    >
      ...
      <div className="mb-8 group">
        <ActionTitle
          title={section.content.actionTitle || "Geographic Intelligence"}
        ...
        <div className="lg:col-span-3">
          <ChartComponent
            config={{
              type: "bar",
              data: locationData,
              title: "Top Location Performance",
              width: 800,
              height: 360,
              xKey: "location",
              yKey: "value",
              ...
              showTooltip: true,
              showLegend: false,
              customRenderer: renderLocationChart,
            }}
            isEditable={false}
          />
        </div>
