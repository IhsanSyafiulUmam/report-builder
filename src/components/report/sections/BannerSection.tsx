          ...
          <tbody className="divide-y divide-gray-100">
            {bannerSummaryRows.map((row, index) => (
              ...
              >
                ...
                {bannerPlatformKeys.map((platformKey) => {
                  const value =
                    dashboardData.bannerSummaries[platformKey][row.key];
                  return (
                    <td
                      key={platformKey}
    ...
    <div className="min-h-[500px] bg-gray-50 flex-1 flex flex-col">
      <div className="flex-1 px-6 py-10 lg:px-12">
        <p className="mb-10 text-base text-gray-600 max-w-8xl">
          Banner performance analysis across major e-commerce platforms by
          banner type. Product banners show the highest engagement with Shopee
          leading at 200 clicks, followed by Tokopedia at 180 clicks.
        </p>
        ...
        <div className="flex items-center justify-between pt-6 text-sm text-gray-500 border-t mt-14">
          <div>
            ...
            <p>
              <span className="font-medium">Channel:</span> Shopee, Tokopedia,
              Lazada, TikTok, Blibli, Bukalapak
            </p>
          </div>
